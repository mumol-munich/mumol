...
class Project(models.Model):  # admin
    ...
    def delete(self, *args, **kwargs):
        ...
        if not self.patientspec.patientdpts_patientspec.exclude(pk = datapointtype_pk) and not self.samplespec.sampledpts_samplespec.exclude(pk = datapointtype_pk):
            self.patientspec.patientdpts_patientspec.all().delete()
            self.samplespec.sampledpts_samplespec.all().delete()
...
class SpecDPTs(models.Model):  # admin
    specification = models.ForeignKey(
        Specification, on_delete=models.RESTRICT, related_name="specdpts_specification")
...
class ConfDPTs(models.Model):  # admin
    chipsetspec = models.ForeignKey(ChipsetSpecification, on_delete = models.RESTRICT, related_name = 'confdpts_chipsetspec', blank = True, null = True) # remove blank and null
    datapointtype = models.ForeignKey(
...
class ProjectId(models.Model):  # user
    ...
    patient = models.ForeignKey(
        Patient, on_delete=models.RESTRICT, related_name="projectid_patient")
...
class Sample(models.Model):  # user
    ...
    def get_samplename(self):
        return f'{self.dateofreceipt.strftime("%d.%m.%Y")} ({self.visit})'
...
class GeneAnalysis(models.Model): # user
    ...
    datapointsrows = models.ManyToManyField(DatapointsRow, related_name="geneanalysis_datapointsrows")
...
#     sample = models.ForeignKey(
#         Sample, on_delete=models.RESTRICT, related_name="chipsetanalysis_sample")
#     chipsetspec = models.ForeignKey(ChipsetSpecification, on_delete = models.RESTRICT, related_name = 'chipsetanalysis_chipsetspec', blank = True, null = True) # remove blank and null
#     confdpts = models.ForeignKey(
...
class PatientSpecification(models.Model):  # admin
    project = models.OneToOneField(
        Project, on_delete=models.CASCADE, related_name="patientspec")

class SampleSpecification(models.Model):  # admin
    project = models.OneToOneField(
        Project, on_delete=models.CASCADE, related_name="samplespec")

class PatientDPTs(models.Model):  # admin
    patientspec = models.ForeignKey(
        PatientSpecification, on_delete=models.RESTRICT, related_name="patientdpts_patientspec")
    datapointtype = models.ForeignKey(
...
class SampleDPTs(models.Model):  # admin
    samplespec = models.ForeignKey(
        SampleSpecification, on_delete=models.RESTRICT, related_name="sampledpts_samplespec")
    datapointtype = models.ForeignKey(
...
class PatientInfo(models.Model): # user
    projectid = models.OneToOneField(
        ProjectId, on_delete=models.CASCADE, related_name="patientinfo")
    patientspec = models.ForeignKey(
...
class SampleInfo(models.Model): # user
    sample = models.OneToOneField(
        Sample, on_delete=models.CASCADE, related_name="sampleinfo")
    samplespec = models.ForeignKey(
...
# def update_profile_signal(sender, instance, created, **kwargs):
def update_project_signal(sender, instance, created, **kwargs):
    if created:
