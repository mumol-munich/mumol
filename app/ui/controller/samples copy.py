...
def samples_overview_add_remove(request, project_pk):
    ...
    if access_type == 'add':
        # add sample
        firstname, lastname, dateofbirth, projectid_val, dateofreceipt, mutation = request.POST.get('firstname'), request.POST.get('lastname'), request.POST.get('dateofbirth'), request.POST.get('projectid'), request.POST.get('dateofreceipt'), request.POST.get('mutation')
        patient_new = False
        # get patient
        try:
        ...
        # get sample
        try:
            sample = Sample.objects.get(projectid_id = projectid.pk, dateofreceipt = dateofreceipt)
        except Exception as e:
            # new sample
            sample = Sample(projectid_id = projectid.pk, dateofreceipt = dateofreceipt)
            if mutation:
                mutation = int(mutation)
                sample.mutation = mutation
            sample.save()
            sample_new = True
        # sampledpts
...
def samples_add_remove(request):
    ...
    access_type = request.POST.get('access_type')
    if access_type == 'add':
        projectid_pk, dateofreceipt, mutation = request.POST.get('projectid_pk'), request.POST.get('dateofreceipt'), request.POST.get('mutation')
        try:
            ...
            # get sample
            try:
                sample = Sample.objects.get(projectid_id = projectid_pk, dateofreceipt = dateofreceipt)
            except Exception as e:
                # new sample
                sample = Sample(projectid_id = projectid_pk, dateofreceipt = dateofreceipt)
                if mutation:
                    mutation = int(mutation)
                    sample.mutation = mutation
                sample.save()
