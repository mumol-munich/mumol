...
def chipset_analysis_add(request, sample_pk):
    ...
    if request.method == 'POST':
        ...
        try:
            confdpts_pks = request.POST.get('custom_pks').split(',')
        except:
            # no datapoints for this analysis
            confdpts_pks = []
        try:
        ...
        if confdpts_pks:
            ...
            try:
                for confdpts_pk in confdpts_pks:
                    ...
                    if not response['ok']:
                        ...
                        messages.error(request, response['message'])
                        return return_page
                    datapoint = ChipsetDatapoint(confdpts_id = confdpts_pk, value = ivalue)
                    datapoint.save()
            ...
            try:
                dpt_pk = fn_create_or_get_none_datapointtype() 
                confdpts = ConfDPTs.objects.get(chipsetspec_id = chipsetspec_pk, datapointtype_id = dpt_pk)
                datapoint = ChipsetDatapoint(confdpts_id = confdpts.pk)
                datapoint.save()
                chipsetanalysis.datapoints.add(datapoint)
                messages.success(request, 'Success')
                return return_page_success
