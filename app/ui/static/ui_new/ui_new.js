...
function AddCustomColorClass() {
    ...
    //     "color2" : "light"
    // }
    $.each(custom_colors, function (key, value) {
        // colors
...
function HighlightProjectActive(project_pk) {
...
// on next button
function GeneSpecificationSetStorage(project_pk, gene_specs_alt, mutation_status_tab, mutation_status_tab_new, next_btn) {
    var itemkey = "gene_specs";
    var myjson = JSON.parse(localStorage.getItem(itemkey) || "{}");
    ...
    if (mutation_status_tab) {
        if (mutation_status_tab_new) {
            ...
            for (var k = 0; k < gene_specs_alt.length; k++) {
    ...
    var that_var = ["id_method", "id_gene"];
    for (var i = 0; i < that_var.length; i++) {
        var that = "#" + that_var[i];
        var that_val = $(that).val();
        var that_next = $(that).parents("li").next("li");
    ...
    for (var i = 0; i < myjson.mutation_status_list.length; i++) {
        var mutation_status_name = myjson.mutation_status_list[i];
        if (mutation_status_tab) {
        ...
        if (myjson.mutation_status_dict[mutation_status_name] === undefined) {
            ...
            // }
            // }
            for (var j = 0; j < myjson.mutation_status_dict[mutation_status_name].dpts.length; j++) {
                var dpt_pk = myjson.mutation_status_dict[mutation_status_name].dpts[j], mutation_status_dpt_name = mutation_status_name + "_" + dpt_pk;
                // check shown
...
// on refresh
function GeneSpecificationGetStorage(project_pk) {
    var itemkey = "gene_specs";
    var myjsondef = {
...
function MutationStatusTabs(myjson, itemkey) {
    var htmlstr = "";
    var htmldivstr = "";
    var mutation_status_tab_index = 0;
    ...
    for (var i = 0; i < myjson.mutation_status_list.length; i++) {
        var mutation_status_name = myjson.mutation_status_list[i], active_var = "", show_var = "", aria_selected_var = "false";
...
function MutationStatusTabsContents(myjson, itemkey, check_shown) {
    for (var i = 0; i < myjson.mutation_status_list.length; i++) {
        var mutation_status_name = myjson.mutation_status_list[i];
        if (myjson.mutation_status_dict[mutation_status_name] !== undefined) {
            for (var j = 0; j < myjson.mutation_status_dict[mutation_status_name].dpts.length; j++) {
                var dpt_pk = myjson.mutation_status_dict[mutation_status_name].dpts[j];
                if (check_shown) {
                    // check shown
                    if (myjson.mutation_status_dict[mutation_status_name].dpts_shown.indexOf(dpt_pk) > -1) {
                        continue;
                ...
                var mutation_status_dpt_name = mutation_status_name + "_" + dpt_pk;
                ...
                var reqspan = $('#custom_datapointtype_span').find('span[data-dpt_pk=' + dpt_pk + ']');
                var reqspan_helptext = reqspan.attr("title");
                var reqspan_options = reqspan.attr("data-options");
                var htmldivstr = '';
...
function CustomSetAction1(itemkey) {
...
function CustomMutationStatusRemoveBtn(itemkey) {
    $(".custom-tabs-mutation-status-tab-delete").on("click", function () {
        // let itemkey: string = "gene_specs";
        var myjson = JSON.parse(localStorage.getItem(itemkey) || "{}");
        // remove from myjson
        var mutation_status_name = $(this).attr("data-name");
        var mutation_status_list_index = myjson.mutation_status_list.indexOf(mutation_status_name);
...
function CustomDptRemoveBtn(itemkey) {
    $(".custom-datapointtypes-li-delete-btn").on("click", function () {
        // let itemkey: string = "gene_specs";
        var myjson = JSON.parse(localStorage.getItem(itemkey) || "{}");
        // remove from myjson
        var mutation_status_name = $(this).attr("data-name"), dpt_pk = $(this).attr("data-dpt_pk");
        var mutation_status_dpt_list_index = myjson.mutation_status_dict[mutation_status_name].dpts.indexOf(dpt_pk);
        ...
        var mutation_status_dpt_shown_list_index = myjson.mutation_status_dict[mutation_status_name].dpts_shown.indexOf(dpt_pk);
...
function CustomDptAddBtn(itemkey) {
    $(".custom-dpt-add-btn").on("click", function () {
        // let itemkey: string = "gene_specs";
        var myjson = JSON.parse(localStorage.getItem(itemkey) || "{}");
        var this_pk = $(this).attr("data-dpt_pk");
        if (myjson.mutation_status_current === "") {
...
//     MutationStatusTabs(myjson, itemkey);
// }
function GeneAnalysisSpecificationStepper(genespecs) {
    ...
    $.each(genespecs, function (i, val) {
        ...
        var htmlstr = '<option value="' + val.method_id + '">' + val.method__name + '</option>';
    ...
    $("#id_method").on("change", function () {
        ...
        $.each(genespecs, function (i, val) {
            ...
            if (String(val.method_id) === String(method_pk)) {
                var htmlstr = '<option value="' + val.gene_id + '">' + val.gene__name + '</option>';
        ...
        var that = $("#custom_stepper_horizontal_ul").find('li').first();
    ...
    $("#id_gene").on("change", function () {
        // add genes
        var method_pk = $("#id_method").val();
        var gene_pk = $(this).val();
        var statuslist = [];
        $.each(genespecs, function (i, val) {
            ...
            if (String(val.method_id) === String(method_pk) && String(val.gene_id) === String(gene_pk)) {
                var htmlstr = '<option value="' + val.pk + '">' + val.status + '</option>';
        ...
        var that = $("#custom_stepper_horizontal_ul").find('li.active');
    ...
    $("#id_status").on("change", function () {
        if ($(this).val() == null || $(this).val() == "") {
            ...
            var searchsubstr = window.location.search.substr(1);
...
function ChipsetSpecificationGetStorage(project_pk) {
    var itemkey = "chipset_specs";
    var myjsondef = {
        "project_pk": project_pk,
        "id_name_chipset": "",
        "id_manufacturer": "",
    ...
    var that_var = ["id_name_chipset", "id_manufacturer", "id_version", "id_genes"];
    var checkvar = true;
    for (var i = 0; i < that_var.length; i++) {
        var that = "#" + that_var[i];
        if (myjson[that_var[i]] != "") {
    ...
    if (checkvar) {
        var that = $(".stepper.stepper-vertical").children("li").first();
    ...
    $.each(myjson.dpts, function (i, val) {
        ...
        var dpt_pk = val, name = myjson.dpts_dict[dpt_pk].name, dpt_name = 'dpt_' + dpt_pk, reqspan_helptext = myjson.dpts_dict[dpt_pk].helptext, reqspan_options = myjson.dpts_dict[dpt_pk].options, mandatory = myjson.dpts_dict[dpt_pk].mandatory, default_val = myjson.dpts_dict[dpt_pk]["default"];
        var htmlstr = '<tr data-pk="' + dpt_pk + '">';
...
function ChipsetSpecificationSetStorage(project_pk) {
    var itemkey = "chipset_specs";
    var myjson = JSON.parse(localStorage.getItem(itemkey) || "{}");
    if (Object.keys(myjson).length === 0) {
    ...
    for (var i = 0; i < myjson.dpts.length; i++) {
        var dpt_pk = myjson.dpts[i], dpt_name = 'dpt_' + dpt_pk;
...
function CustomChipsetDptsAddBtn() {
    ...
    $(".custom-dpt-add-btn").on("click", function () {
        ...
        var reqspan = $(this);
        var dpt_name = 'dpt_' + reqspan.attr("data-dpt_pk"), reqspan_helptext = reqspan.attr("title"), reqspan_options = reqspan.attr("data-options"), name = reqspan.html(), dpt_pk = reqspan.attr("data-dpt_pk");
...
function CustomChipsetDptsRemoveBtn(that) {
    var itemkey = "chipset_specs";
    var myjson = JSON.parse(localStorage.getItem(itemkey) || "{}");
    var dpt_pk = $(that).parents("tr").attr("data-pk");
    $(that).parents("tr").remove();
    var dpt_list_index = myjson.dpts.indexOf(dpt_pk);
...
function SetLatestProject(project_pk, user_pk) {
    if (project_pk) {
        localStorage.setItem('latest_project_id', project_pk);
        localStorage.setItem('latest_project_user', user_pk);
    }
}
function ProjectRedirectFunc(project_redirect, first_project_id) {
    if (project_redirect == "true") {
        var latest_project_id = localStorage.getItem('latest_project_id') || '';
        var latest_project_user = localStorage.getItem('latest_project_user') || '';
...
function PatientSpecificationGetStorage(project_pk) {
    var itemkey = "patient_specs";
    var myjsondef = {
        "project_pk": project_pk,
        "dpts": [],
        "dpts_dict": {}
...
function CustomPatientDptsRemoveBtn(that) {
    var itemkey = "patient_specs";
    var myjson = JSON.parse(localStorage.getItem(itemkey) || "{}");
    var dpt_pk = $(that).parents("tr").attr("data-pk");
    $(that).parents("tr").remove();
...
function SampleSpecificationGetStorage(project_pk) {
    var itemkey = "sample_specs";
    var myjsondef = {
        "project_pk": project_pk,
        "dpts": [],
        "dpts_dict": {}
...
function CustomSampleDptsRemoveBtn(that) {
    var itemkey = "sample_specs";
    var myjson = JSON.parse(localStorage.getItem(itemkey) || "{}");
    var dpt_pk = $(that).parents("tr").attr("data-pk");
    $(that).parents("tr").remove();
...
function PatientSpecificationSetStorage(project_pk) {
    var itemkey = "patient_specs";
    var myjson = JSON.parse(localStorage.getItem(itemkey) || "{}");
    if (Object.keys(myjson).length === 0) {
...
function SampleSpecificationSetStorage(project_pk) {
    var itemkey = "sample_specs";
    var myjson = JSON.parse(localStorage.getItem(itemkey) || "{}");
    if (Object.keys(myjson).length === 0) {
