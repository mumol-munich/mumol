...
# process = subprocess.Popen([f'{binpath}/python', '-m', 'nodeenv', '-p'], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
# process = subprocess.Popen([f'{binpath}/nodeenv', '-p'], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
process = subprocess.Popen([f'{binpath}/python', '-m', 'nodeenv', '-p'], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
stdout, stderr = process.communicate()
print(stdout)
print(stderr)
...
# installing npm
os.chdir(f'{datamask}/app/ui/static')
print('Installing required node_modules...')
cenv = deepcopy(os.environ)
cenv['PATH'] = f"{binpath}:{cenv['PATH']}"
# process = subprocess.Popen([f'{binpath}/npm', 'install'], stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell = shell_switch)
process = subprocess.Popen([f'{binpath}/npm', 'install'], stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell = shell_switch, env = cenv)
# shell = True works in windows and shell = False works in linux
# process = subprocess.Popen([f'{binpath}/npm', 'install', '--prefix', f'{datamask}/app/ui/static'], stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell = True)
stdout, stderr = process.communicate()
print(stdout)
print(stderr)
os.chdir(f'{datamask}/app')
